...
public class Parser {
	...
	public Stack<String> ariSEM = new Stack<String>();//表达式栈
	public int tagX = 0;
	public int tagB = 0;
	public int tagR = 0;
	...
	public boolean rightSquare=true;
...
//		  }
	...
	private boolean ariF() {//F->I{PUSH(I)}|(E)子程序
		...
		if(w.equals("(")){
			...
			System.out.println("N2()Success");
			System.out.println(ariSEM.get(ariSEM.size()-1)+"  "+ariSEM.get(ariSEM.size()-2));
			return true;
		}
		else
			return flag;
	...
	public boolean N3(){
		...
		if(SymbolTable.name.contains(w)){
			...
			System.out.println(w);
			if(N4()){
				System.out.println("N4()Seccess  "+dimensionSize);
				if(dimensionSize!=0&&nullFlag&&rightSquare){
					Stack<String> stack=new Stack<String>();
					String s="";
					while(dimensionSize!=0){
						stack.push(ariSEM.pop());
						dimensionSize--;
					}
					stack.push(ariSEM.pop());
					s=stack.pop();
					...
					if(w.equals("}")){
						if(sizeVALUE > sizeARRAY){
							...
							//处理char类型数组的初始化
								...
								System.out.println("字符串长度为" + sizeVALUE);
								for(int index = sizeARRAY - 1; index > sizeVALUE - 1; index--){
									Quadruples.quadruples[Quadruples.count++] = new Quadruples("=", "", "$", var + index);
								}
							}
						}
					}
	...
	public boolean K(){
		...
		if(w.equals("\"")){
			if(judgeType == 1){
				System.out.println("赋值类型成功");
				w = Search(i++);
				System.out.println(w);
				if(CharacterTable.character.contains(w)){
					sizeVALUE = sizeVALUE -1 +w.length();
					System.out.println("已存储的字符串为" + w);
					w = Search(i++);
					System.out.println(w);
					if(w.equals("\"")){
						w = Search(i++);
				...
				System.out.println("赋值类型出错");
				return false;
			}
		}
		else if(NumberTable.number.contains(w)){
			System.out.println("数组类型为" + judgeType);
			if(judgeType == 0){
				ariSEM.push(w);
				w = Search(i++);
				System.out.println(w);
